...
admin.autodiscover()
from meet.views import UserProfileDetailView, dashboard, home, friends, create_event, event_detail, event_edit, profile, profile_edit, profile_events, user_invite
from api.views import get_friends, invite
...
#    url(r'^i$', 'meet.views.accountform', name='accountform'),
    ...
    url(r'^invite/(?P<timestamp>\d+)/(?P<request_id>[\w-]+)/$', user_invite, name="user_invite"), 

    #### API #####    
    url(r'^api/friends/$', get_friends, name="get_friends"), 
    url(r'^api/invite/$', invite, name="invite"), 
